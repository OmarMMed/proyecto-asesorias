...
if(isset($_SESSION['numCuenta'])){
  ...
  if($total==0){
    $total = mysqli_num_rows(mysqli_query($conn,"SELECT idAsesore FROM asesores WHERE idAsesore='$numCuenta'"));
    if($total==0){
      header("Location: home_rt.php");
    }else{
      header("Location: home_asesores.php");
    }
  }else{
